  ...
  const fetchAlerts = async (status = "", type = "") => {
    try {
      ...
      const mapped = rawAlerts.map((a) => ({
        ...
        dueDays: a.due_days ?? "—",
        assignTo: a.assigned_user_name ? String(a.assigned_user_name) : "—",
      }));
...
}, [alerts, searchTerm, selectedDeviceType, selectedPriority, fromDate, toDate, alertIdSortOrder, selectedClient, selectedLocation]);
      ...
      <div className="w-full">
        <div className="flex justify-between items-center select-none">
          ...
          {filteredAlerts.length > 0 && (
            <button
              type="button"
              className="download-alert-button flex items-center gap-2"
              onClick={exportAlerts}
              title="Download selected alerts or all alerts as CSV"
            >
              <img src="src/assets/img/alerts/excel.svg" alt="excel" />
              Download Alert
            </button>
          )}
        </div>
...
<div className="selection-filter">
  ...
  {(searchTerm || selectedAlertType || selectedStatus || selectedClient || selectedLocation || fromDate || toDate || selectedDeviceType || selectedPriority || alertIdSortOrder) && (
    ...
    >
      ...
      {/* ==== TABLE ==== */}
      <div className="alert-table-container" style={{ overflowY: "visible", overflowX: "auto" }}>
        {paginated.length === 0 ? (
              ...
              <tbody>
                {paginated.length ? (
                  paginated.map((a, i) => {
                    ...
                    return (
                      <tr key={a.alertId} className="alert-table-row">
                        ...
                        <td className="alert-table-cell">{a.did}</td>
                        <td className="alert-table-cell">{a.location}</td>
                        <td className="alert-table-cell">{a.alertDescription}</td>
                        ...
                        <td className="alert-table-cell">{a.status}</td>
                      </tr>
                    );
                  })
                ) : (
