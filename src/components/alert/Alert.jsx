...
console.log("🧾 /user-clients raw response:", json);
  ...
  json.map((c) => ({
    ...
    clientName: c.client?.name ?? c.client_name ?? c.clientName ?? c.name,
  }))
);
    ...
    const fetchAlerts = async (status = "", type = "") => {
      try {
        ...
        console.log("🔍 Fetching alerts:", { url, hasToken: !!token, tokenLength: token?.length });
...
raisedOn: a.raised_on
  ? new Date(a.raised_on).toLocaleString()
  : "—",

raisedOnRaw: a.raised_on
  ? new Date(a.raised_on).getTime()
  : null,

  closedOn: a.closed_on
    ? new Date(a.closed_on).toLocaleString()
    : "—",
  ...
  assignTo: a.assigned_user_name || "—",
    ...
    neon:  { Icon: LuLightbulb, label: "NEON", bg: "bg-gradient-to-tl from-yellow-600 via-yellow-500 to-yellow-400" },
    unknown: { Icon: MdDevicesOther, label: "Unknown", bg: "bg-gradient-to-tl from-gray-600 via-gray-500 to-gray-400" },
        ...
        alert("Failed to load alerts. Please check your connection and try again.");
      } finally {
        setLoading(false);
      }
    };
  ...
  setRaisedOnSortOrder("desc"); // newest first by default
    ...
    if (selectedLocation) {
      data = data.filter((a) => String(a.locationId) === String(selectedLocation));
    }
    ...
    if (selectedDeviceType) {
      data = data.filter((a) => a.deviceType.toLowerCase() === selectedDeviceType.toLowerCase());
    }
...
if (raisedOnSortOrder) {
  ...
  selectedLocation,
    ...
    useEffect(() => setCurrentPage(1), [searchTerm, selectedAlertType, selectedStatus, selectedDeviceType]);
...
>
  ...
  <div className="selection-filter"  style={{
    marginBottom: "-40px",
    display: "flex",
    alignItems: "center",
    ...
    {(searchTerm || selectedAlertType || selectedStatus || selectedClient || selectedLocation || fromDate || toDate || selectedDeviceType || selectedPriority || alertIdSortOrder) && (
      <button
        onClick={() => {
          setSearchTerm("");
  ...
  {filteredAlerts.length > 0 && (
    ...
    >
      <FiDownload size={24} />
        ...
        {/* ==== TABLE ==== */}
        <div className="alert-table-container hide-scrollbar  center-table" style={{ width: '100vw', maxWidth: '100%', marginLeft: '-920px', marginTop:'-20px', left: '50%', position: 'relative', right: '50%', overflowX: 'auto', overflowY: 'visible' }}>
          {paginated.length === 0 ? (
            ...
            <>
              {/* Sticky Table Header */}
              <div style={{ width: '100%', minWidth: '1200px', position: 'sticky', top: 0, zIndex: 10, background: '#0f172b' }}>
                <table  classname="hide-scrollbar"style={{ width: '100%', tableLayout: 'fixed' }}>
                  <thead className="bg-[#0f172b] h-10 w-full">
                    <tr>
                      ...
                      <th className="alert-table-header" style={{ minWidth: '180px', maxWidth: '240px', width: '200px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Alert Type</th>
  ...
  <div className="flex items-center justify-center gap-1">
    ...
    {raisedOnSortOrder === "asc" && (
      <IoIosArrowUp size={14} />
              ...
              <div className="overflow-x-auto  hide-scrollbar" style={{ width: '100%', minWidth: '100%', maxHeight: '60vh', overflowY: 'auto' }}>
                <table className="alert-table-body rounded-xl" style={{ width: '100%', minWidth: '1200px', tableLayout: 'fixed' }}>
                  <tbody>
                    {paginated.length ? (
                      paginated.map((a, i) => {
                        ...
                        return (
                          <tr
                           ...
                           onClick={() =>
                             navigate(`/alerts/${a.alertId}`, { state: { alert: a } })
                                    }
                          ...
                          >
                            ...
                            <td className="alert-table-cell" style={{ minWidth: '180px', maxWidth: '240px', width: '200px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{a.alertType}</td>
                            <td className="alert-table-cell">{a.deviceType}</td>
                            <td className="alert-table-cell one-line-text">{a.alertTitle}</td>
                            ...
                            <td className="alert-table-cell ">
                              <span
                                ...
                                style={{ whiteSpace: 'nowrap', minWidth: 80, display: 'inline-block', textAlign: 'center', color: '#fff' }}
                              >
                                {a.priority}
                              </span>
                            </td>
                    ...
                    <tr>
                      <td colSpan="11" className="p-4">
                        <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 w-full">
                          ...
                          <div className="flex items-center gap-2 text-white">
                            <button onClick={goFirst} disabled={currentPage === 1} className="pagination-btn" title="First">
                              <TfiControlSkipBackward />
                            </button>
                            <button onClick={goPrev} disabled={currentPage === 1} className="pagination-btn" title="Prev">
                              <IoIosArrowBack />
                            </button>
                            <button onClick={goNext} disabled={currentPage === totalPages} className="pagination-btn" title="Next">
                              <IoIosArrowForward />
                            </button>
                            <button onClick={goLast} disabled={currentPage === totalPages} className="pagination-btn" title="Last">
                              <TfiControlSkipForward />
                            </button>
                          </div>
                        </div>
